...
defaultfunction = ['exp','sin','cos','tan','csc'] #기본적으로 sympy에서 정의된 함수들 
...
#                    lastinsertidx = insertidx[col] =  len(param_list) +1 if hasparam and repeat_flag else len(param_list)
                ...
                if wasjumped:
                    for ii in jumprange:
                        ...
                        if not ((not isinstance(noparam[ii], list) or not ii in noparam[ii][1]) and loopback[ii] == ii) or isinstance(parsed[col],list):
